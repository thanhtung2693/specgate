...
├── eval_contract.py         # contract judges (FixtureJudge + LiveJudge) + schemas
...
# Run one target end-to-end (deterministic fixture judge; no provider keys):
...
The default `--contract-judge fixture` mode is a deterministic baseline;
`--contract-judge live` calls the settings-backed governance model. Each row
scores against two assertions:
...
Live calibration mode (uses settings-backed governance model/provider keys):
...
`score_*` function returning `(score, diagnostics)`, deterministic fixture-judge
tests for CI, and one opt-in `live_smoke` calibration test.

- `evals/quality_gates/` — the five model-judged quality gates evaluated by
...
|---|---|---|
| Contract schema or scorer changes | `uv run python -m evals.run --target quality_gate_contract --contract-judge fixture --json`<br/>`uv run python -m evals.run --target ac_satisfaction_contract --contract-judge fixture --json` | Both targets must meet threshold (`threshold_met=true`) |
...
The contract targets default to the deterministic fixture judge, so a
`--target all` run costs nothing. `--contract-judge live` calls the
configured governance model once per row (six rows total), staying well under
a cent at current pricing.
