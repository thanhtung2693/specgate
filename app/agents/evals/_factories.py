...
def build_main_model() -> BaseChatModel:
    """Construct the settings-backed governance model via the production path."""
    try:
...
# Non-chat governance workloads use one settings-backed model; the harness keeps
# both builder names so older eval rows that ask for a "mini" model resolve to
# the same model.
