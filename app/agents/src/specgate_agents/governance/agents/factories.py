...
def _is_google_provider_name(provider: str) -> bool:
    return provider.lower().startswith("google")
...
def _provider_chat_kwargs(
    ...
    *,
    ...
    if provider == "openai":
        ...
        # gpt-5 is a reasoning model and takes an explicit effort; other OpenAI
        # models ignore reasoning and keep just the temperature.
        if model_id.lower().startswith("gpt-5"):
            kwargs["reasoning_effort"] = level
            kwargs["use_responses_api"] = True
        return kwargs

    if _is_google_provider_name(provider):
        kwargs["temperature"] = temperature
...
def build_model(*, thinking_level: str | None = None) -> Any:
    """The app's single chat model, used for every governance workload.
    ...
    return init_chat_model(
        ...
        **_configured_provider_chat_kwargs(provider, model_id, thinking_level=level),
    )
...
def build_governance_ops_model() -> Any:
    ...
    Isolated from the main governance model so governance-ops never shares a
    model instance with the coding agent that produced the artifact being
    reviewed. Configured via GOVERNANCE_OPS_MODEL / GOVERNANCE_OPS_MODEL_PROVIDER
    / GOVERNANCE_OPS_API_KEY / GOVERNANCE_OPS_THINKING_LEVEL env vars; defaults
    to gpt-5.4-mini at low reasoning (fast, cheap, support-only workload).
    """
    ...
    return init_chat_model(
        model_id,
        model_provider=provider,
        **_provider_chat_kwargs(provider, model_id, api_key=api_key, thinking_level=level),
    )


def ensure_llm_env() -> bool:
    """Return True if the configured model provider's API key is present."""
    return provider_has_api_key(governance_model_provider())
