"""Feature reference-attachment helpers shared by the Doc Registry coding-agent
Context Pack and the quality-gate judge.
...
or ``both`` — that controls who sees it. Doc Registry renders ``coding_agent`` /
``both`` in Context Packs; the gate renders ``gate`` / ``both``. The default at
