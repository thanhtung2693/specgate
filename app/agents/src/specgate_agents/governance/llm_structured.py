...
def _prefers_function_calling_structured_output(model: Any) -> bool:
    """Use OpenAI function-calling structured output to keep traces serializable.

    Extended to OpenRouter because ChatOpenRouter extends ChatOpenAI and its
    with_structured_output accepts a ``method`` parameter — but its module prefix
    differs from langchain_openai.
    """
    module = type(model).__module__
    if not (module.startswith("langchain_openai") or module.startswith("langchain_openrouter")):
        return False
    try:
...
async def structured_output_ainvoke[T: BaseModel](
    ...
    *,
    ...
    if _prefers_function_calling_structured_output(model):
