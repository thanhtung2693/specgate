...
# The app runs on a single model for every workload (classification, summaries,
# drafting, judges, gates). Selected by `governance.model_provider` / `governance.model`.
...
# overrides them, invalid/blank values keep the default. Confidence thresholds
# gate the LLM judge downgrades; the operational knobs tune artifact publish.
_GATE_CONFIDENCE_THRESHOLD_DEFAULT = 0.7
_PUBLISH_RETRY_ATTEMPTS_DEFAULT = 3
_PUBLISH_RETRY_BASE_SECONDS_DEFAULT = 0.5
...
_publish_retry_attempts = _PUBLISH_RETRY_ATTEMPTS_DEFAULT
_publish_retry_base_seconds = _PUBLISH_RETRY_BASE_SECONDS_DEFAULT
...
def _hydrate_from_doc_registry_sync() -> None:
    ...
    populated = sorted(_provider_api_keys.keys())
    if populated:
        logger.info("provider_settings_hydration: cached keys for providers=%s", populated)
    else:
        logger.warning(
...
def set_provider_api_keys_from_settings(settings: dict[str, str]) -> None:
    ...
    global _governance_model_name, _governance_model_provider
...
def _apply_runtime_policy_settings(settings: dict[str, str]) -> None:
    ...
    global _publish_retry_attempts, _publish_retry_base_seconds, _registry_timeout_seconds
    ...
    _publish_retry_attempts = _coerce_int(
        settings.get("governance.publish_retry_attempts"),
        _PUBLISH_RETRY_ATTEMPTS_DEFAULT,
    )
    _publish_retry_base_seconds = _coerce_float(
        settings.get("governance.publish_retry_base_seconds"),
        _PUBLISH_RETRY_BASE_SECONDS_DEFAULT,
    )
...
def governance_publish_retry_attempts() -> int:
    return _publish_retry_attempts


def governance_publish_retry_base_seconds() -> float:
    return _publish_retry_base_seconds
