...
Every LLM gate follows the same shape: a prompt template, a gate-name
...
ROLLBACK_PLAN_PROMPT = (
    "You are reviewing a software change's planning artifact for one quality gate:\n"
...
async def _judge(
    ...
    *,
    ...
    prompt = prompt_template.format(
        artifact=artifact_md.strip(), work_type=work_type.strip() or "unspecified"
    )
...
async def evaluate_all_gates(
    ...
    *,
    ...
    """Run all LLM gates concurrently against routed artifact sections.
    ...
    # gate_skills (from the profile snapshot) bind a rubric Skill prompt to a gate;
    # an unbound gate gets None and runs with its built-in prompt (graceful fallback).
