...
def _att(**kw):
    base = {
        "kind": "link", "url": "", "governance_file_id": "",
        "title": "", "note": "", "audience": "gate",
    }
    base.update(kw)
    return base
...
def test_context_pack_includes_coding_agent_attachments_only() -> None:
    from specgate_agents.governance.board.context_pack import render_context_pack

    md = render_context_pack(
        change_request={"intent_md": "do x"},
        feature={"key": "feat-1"},
        warnings=[],
        artifact_bundle={"prd": "PRD body"},
        attachments=[
            _att(
                audience="coding_agent", kind="link",
                url="https://ex.com/follow", title="Example",
            ),
            _att(audience="gate", kind="link", url="https://ex.com/bug", title="Bug only for gate"),
        ],
    )
    assert "https://ex.com/follow" in md  # coding_agent → in handoff
    assert "https://ex.com/bug" not in md  # gate-only → never in handoff
...
def test_gate_threads_gate_audience_attachments_into_edge_and_scope(monkeypatch) -> None:
    ...
    bundle = {"prd": "P", "spec": "S", "tasks_qa": "Q", "rollout": "R", "risks": "K"}
    atts = [
