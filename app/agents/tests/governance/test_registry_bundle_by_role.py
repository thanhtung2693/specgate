...
async def test_aload_artifact_bundle_by_role_stable_sort_order() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if "path" in params:
            file_path = params["path"]
            text = file_texts.get(file_path, "")
            return httpx.Response(200, json={"body": {"content": text}})
...
async def test_aload_artifact_bundle_by_role_missing_role_becomes_unspecified() -> None:
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        ...
        if "path" in params:
            return httpx.Response(200, json={"body": {"content": "# Readme"}})
...
@pytest.mark.asyncio
async def test_aload_artifact_bundle_by_role_rejects_oversized_inline_body() -> None:
    files_items = [{"path": "spec.md", "role": "spec", "size_bytes": 10}]

    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path == "/artifacts/art-1/files":
            return httpx.Response(200, json={"items": files_items})
        if request.url.path == "/artifacts/art-1/files/_":
            return httpx.Response(
                200,
                json={"body": {"content": "x" * ((1 << 20) + 1)}},
            )
        return httpx.Response(404)

    client = _make_client(handler)

    assert await client.aload_artifact_bundle_by_role("art-1") == {}


@pytest.mark.asyncio
async def test_aload_artifact_bundle_by_role_rejects_oversized_package() -> None:
    files_items = [
        {"path": f"doc-{index}.md", "role": "spec", "size_bytes": 1 << 20} for index in range(11)
    ]
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path == "/artifacts/art-1/files":
            return httpx.Response(200, json={"items": files_items})
        if request.url.path == "/artifacts/art-1/files/_":
            return httpx.Response(200, json={"body": {"content": "x" * (1 << 20)}})
        return httpx.Response(404)

    client = _make_client(handler)

    with pytest.raises(ValueError, match="10 MiB"):
        await client.aload_artifact_bundle_by_role("art-1")
