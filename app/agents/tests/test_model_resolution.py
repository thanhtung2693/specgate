"""Single-model resolution for every governance workload.

The app runs on one model (`governance.model_provider` / `governance.model`) for all
workloads — classification, summaries, drafting, judges, and gates. There is no
mini/main/system tier split: `build_model()` and `ensure_llm_env()` resolve the
...
def test_build_governance_ops_model_requires_generic_api_key_env(monkeypatch) -> None:
    ...
    monkeypatch.setenv("OPENROUTER_API_KEY", "sk-provider-specific")
    monkeypatch.setattr(factories, "provider_api_key_kwargs", _should_not_read_provider_key)
...
def test_build_model_openai_gpt5_sets_reasoning_effort(monkeypatch) -> None:
    kwargs = _capture_build_kwargs(
        monkeypatch, provider="openai", model_id="gpt-5.4-nano", level="high"
    )
...
def test_workload_call_sites_use_the_single_builder() -> None:
    """The judges / classifiers / drafters / gates all build the one configured model.
    ...
    """
    from specgate_agents.governance.board import (
        quality_gates,
        route_suggestion,
    )
    ...
    for module in (title, route_suggestion, quality_gates):
    ...
    # The readiness gates use the configured model, not the governance-ops chat model.
    qg_src = inspect.getsource(quality_gates)
    assert "build_governance_ops_model()" not in qg_src, (
        "quality_gates must use the configured build_model(), not the governance-ops model"
    )
