"""Runtime policy/operational settings hydrated from Doc Registry.

The gate confidence threshold and the publish-retry / registry-timeout
operational knobs are user-configurable (settings table) instead of hardcoded
constants. They ride the same hydration path as provider keys
...
def test_defaults_match_legacy_constants() -> None:
    ...
    assert pk.governance_publish_retry_attempts() == 3
    assert pk.governance_publish_retry_base_seconds() == 0.5
    ...
    pk.set_provider_api_keys_from_settings(
        {
            ...
            "governance.publish_retry_attempts": "5",
            "governance.publish_retry_base_seconds": "1.5",
            "governance.registry_timeout_seconds": "45",
        }
    )
    assert pk.governance_gate_confidence_threshold() == 0.85
    assert pk.governance_publish_retry_attempts() == 5
    assert pk.governance_publish_retry_base_seconds() == 1.5
    ...
    pk.set_provider_api_keys_from_settings(
        {
            ...
            "governance.publish_retry_attempts": "",
        }
    )
    assert pk.governance_gate_confidence_threshold() == 0.7
    assert pk.governance_publish_retry_attempts() == 3
