...
// ListGateTasks calls GET /api/v1/gate-tasks?workspace_id=<workspaceID>&artifact_id=<artifactID>.
func (c *Client) ListGateTasks(ctx context.Context, artifactID string) ([]GateTask, error) {
	var out struct {
		Tasks []GateTask `json:"tasks"`
	}
	q := url.Values{"artifact_id": {artifactID}}
	if workspaceID := WorkspaceID(ctx); workspaceID != "" {
		q.Set("workspace_id", workspaceID)
	}
	if err := c.get(ctx, "/api/v1/gate-tasks?"+q.Encode(), &out); err != nil {
		return nil, err
	}
	return out.Tasks, nil
}

// GetGateTask calls GET /api/v1/gate-tasks/{task_id}?workspace_id=<workspaceID>.
func (c *Client) GetGateTask(ctx context.Context, taskID string) (*GateTask, error) {
	var task GateTask
	path := "/api/v1/gate-tasks/" + url.PathEscape(taskID)
	if workspaceID := WorkspaceID(ctx); workspaceID != "" {
...
// SubmitGateResult calls POST /api/v1/gate-tasks/{task_id}/result?workspace_id=<workspaceID>.
func (c *Client) SubmitGateResult(ctx context.Context, taskID string, body any) (*GateResultResponse, error) {
	...
	path := fmt.Sprintf("/api/v1/gate-tasks/%s/result", url.PathEscape(taskID))
	if workspaceID := WorkspaceID(ctx); workspaceID != "" {
		path += "?" + url.Values{"workspace_id": {workspaceID}}.Encode()
	}
