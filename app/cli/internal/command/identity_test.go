...
func TestUserLoginRejectsLegacyNameAlias(t *testing.T) {
	t.Parallel()
	deps, fc, _, out := newFakeDeps(t)
	deps.ConfigPath = filepath.Join(t.TempDir(), "config.json")
	fc.identitySelection = &client.IdentitySelection{
		User:      client.IdentityUser{ID: "user-1", Username: "newuser", DisplayName: "New User"},
		Workspace: client.IdentityWorkspace{ID: "ws-1", Slug: "fresh-workspace", Name: "Fresh Workspace"},
	}
	...
	if !strings.Contains(out.String(), `"command":"user.login"`) ||
		...
		!strings.Contains(out.String(), "unknown flag: --name") {
		t.Fatalf("output = %q, want structured usage error for legacy flag", out.String())
	}
	if fc.calls != 0 {
