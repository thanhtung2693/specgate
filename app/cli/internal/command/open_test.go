...
func TestOpenPrintWorkURLWithoutLaunchingBrowser(t *testing.T) {
	t.Parallel()
	deps, fc, opened, out := newOpenDeps(t)
	code := command.ExecuteForCode(command.NewRootCommand(deps), "--plain", "--server", "http://web.test", "open", "CR-101", "--print")
	if code != output.ExitOK {
		t.Fatalf("exit = %d, output = %s", code, out.String())
	}
	if fc.lastWorkRef != "CR-101" {
	...
	if *opened != "" {
		t.Fatalf("opened = %q, want no browser side effect", *opened)
	}
	if got := strings.TrimSpace(out.String()); got != "http://web.test/work/CR-101" {
		t.Fatalf("output = %q", got)
	}
}

func TestOpenPrintArtifactJSONWithoutLaunchingBrowser(t *testing.T) {
	t.Parallel()
	deps, _, opened, out := newOpenDeps(t)
	code := command.ExecuteForCode(command.NewRootCommand(deps), "--json", "--server", "http://web.test", "open", "--artifact", "art-1", "--print")
	if code != output.ExitOK {
		t.Fatalf("exit = %d, output = %s", code, out.String())
	}
	if *opened != "" {
	...
	if envelope.Data.URL != "http://web.test/artifacts?artifact=art-1" {
		t.Fatalf("url = %q", envelope.Data.URL)
	}
}

func TestOpenPrintDeepLinkRequiresAdvertisedWebURL(t *testing.T) {
	t.Parallel()
	deps, fc, opened, out := newOpenDeps(t)
	fc.metaResult = &client.Meta{APIVersion: "specgate.api/v1"}
	code := command.ExecuteForCode(command.NewRootCommand(deps), "--plain", "open", "CR-101", "--print")
	...
	if !strings.Contains(out.String(), "web UI URL") {
		t.Fatalf("output = %q, want missing web_url guidance", out.String())
	}
}
...
func TestUnreachableServerErrorMessage(t *testing.T) {
	...
	code := command.ExecuteForCode(command.NewRootCommand(deps), "--plain", "--server", serverURL, "status", "--all-workspaces")
	if code != output.ExitUnavailable {
		t.Fatalf("exit = %d, want %d, output = %s", code, output.ExitUnavailable, out.String())
	}
	want := "SpecGate server unreachable at " + serverURL +
		" — run `specgate doctor`. If this IDE agent runs in a restricted sandbox, rerun it with local-network access; do not start Docker unless the user asks."
	if !strings.Contains(out.String(), want) {
		t.Fatalf("output = %q, want %q", out.String(), want)
	}
}

func TestUnreachableServerJSONEnvelope(t *testing.T) {
	t.Parallel()
	srv := httptest.NewServer(http.HandlerFunc(func(http.ResponseWriter, *http.Request) {}))
	serverURL := srv.URL
	srv.Close() // guarantee connection refused
	...
	code := command.ExecuteForCode(command.NewRootCommand(deps), "--json", "--server", serverURL, "status", "--all-workspaces")
	if code != output.ExitUnavailable {
		t.Fatalf("exit = %d, want %d, output = %s", code, output.ExitUnavailable, out.String())
	}
	var env struct {
	...
	if env.OK || env.Error.Code != "unavailable" {
		t.Fatalf("envelope = %#v, want unavailable error", env)
	}
	if !strings.Contains(env.Error.Message, "run `specgate doctor`") {
		t.Fatalf("message = %q, want doctor guidance", env.Error.Message)
	}
}
