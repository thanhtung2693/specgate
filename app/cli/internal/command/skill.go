...
func newSkillListCmd(deps *Deps) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, _ []string) error {
			skills, err := deps.Client.ListSkills(cmd.Context(), nameFilter)
			if err != nil {
				code := deps.Printer.Error("skill.list", mapAPIError("skill.list", err))
			...
			if len(skills) == 0 {
				fmt.Fprintln(deps.Stdout, "No skills found.")
				return nil
			}
...
func newSkillShowCmd(deps *Deps) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			s, err := deps.Client.GetSkill(cmd.Context(), query)
			if err != nil {
				// Fall back to name search.
				skills, listErr := deps.Client.ListSkills(cmd.Context(), query)
				if listErr != nil {
					code := deps.Printer.Error("skill.show", mapAPIError("skill.show", err))
					return &output.ExitError{Code: code, Err: err}
				}
			...
			if s.Description != "" {
				fmt.Fprintf(deps.Stdout, "Desc: %s\n", s.Description)
			}
			if s.Prompt != "" {
