...
func withSkillWorkspace(t *testing.T, deps *command.Deps) {
	t.Helper()
	if err := (config.Config{Workspace: config.CurrentWorkspace{ID: "ws-a", Slug: "platform"}}).SaveTo(deps.ConfigPath); err != nil {
		t.Fatal(err)
	}
}
