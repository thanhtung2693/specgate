...
func TestStatsRendersGovernanceValueReadout(t *testing.T) {
	...
	for _, want := range []string{
		...
		"2 gate failures before handoff",
		"3 review failures (3 fixed before merge)",
		"4 resubmits across 2 items",
		"1 blocked-ambiguity report",
		"avg 6.2h create → pass (3 items)",
		"Caught by SpecGate (recent)",
		"CR-EB7FC5BB",
		"review_catch",
...
func TestStatsJSONEmitsRawPayloadEnvelope(t *testing.T) {
	...
	code := command.ExecuteForCode(command.NewRootCommand(deps), "--json", "stats")
	if code != output.ExitOK {
		t.Fatalf("exit = %d, output = %s", code, out.String())
	}
	var env struct {
