...
func TestPeerReviewRejectsSkippedBoundCheck(t *testing.T) {
	store, workspaceID, work := boundWork(t, "Retries stop after three attempts @check:unit")
	...
	peer := boundCompletion(work, map[string]any{"name": "unit", "status": "skipped", "detail": "peer did not run it"})
	peer["agent"] = map[string]any{"name": "reviewer"}
	peer["peer_review_of"] = map[string]any{
		"completion_feedback_event_id": completion.ID,
		"git_receipt":                  receipt,
	}

	_, err = store.PeerReviewDelivery(context.Background(), workspaceID, work.Key, peer)
	if err == nil || !strings.Contains(err.Error(), "unit") {
		t.Fatalf("peer review error = %v, want rejection naming the bound check", err)
	}
}
