...
Append-only feedback signals from coding agents, delivery systems, and linked trackers. A signal may prompt someone to open an artifact-update proposal on demand, but the event row itself remains immutable history.
...
| `status` | TEXT | Persisted as `pending | processed | ignored`; higher-level UI may label these as `received | accepted | rejected`. A proposal verdict reconciles the linked row (`pending → processed` on approve, `pending → ignored` on reject) |
...
- `get_governance_status()` — no input required. Returns an aggregate governance snapshot in two queries (all CRs + all stale warnings). Output: `{ summary, counts: { intake, draft, review, ready, handoff, delivered, total }, attention: [{ change_request_id, key, title, phase, issues }] }`. `attention` lists only CRs with at least one stale warning; empty slice means no issues. Delivered CRs (latest `delivery_review` gate run passed) are counted under `delivered` — not `handoff`/`ready` — and never appear in `attention`. Intended as the first call at session start to orient the developer before picking up a work item.
...
Board phase is derived on read from ChangeRequest artifact pointers, plus a server-side `Delivered` override when the CR's latest `delivery_review` gate run passed. No separate ChangeRequest status is stored.
...
| `POST /artifacts/{id}/readiness-runs/refresh` | Persist an artifact-scoped readiness snapshot and return rows. Request body is optional (`{}` or empty), with optional `evaluations[]` (`gate`, `state`, `hint`, `confidence`, `judge_model`, `eval_suite_version`, `evidence`) from the artifact-readiness runner. Judge metadata (`judge_model`, `confidence`, `eval_suite_version`) is preserved by wrapping the evidence into the `gate-run-v1` envelope in `evidence_json` unless the caller already supplied an envelope. Returns newest-ready rows (`gate`, `state`, `hint`, `evidence_json`, `created_at`) for that artifact. |
| `GET /artifacts/{id}/readiness-runs?limit=` | List persisted artifact-scoped readiness rows for one artifact (newest first), across all executors; each row carries `executor` (`platform` or `ide_agent`) so history shows trust origin. The platform-only filter applies to the readiness approval aggregate, not this history view. |
...
`feature_summary_outdated` is emitted for a feature when it has a persisted Overview (`summary_md` non-empty) whose `summary_source_version` differs from the current canonical artifact `version`. Severity is `warning`. It clears when a fresh Overview is written against the current canonical version via `PUT /workboard/features/{id}/summary`, which sets both `summary_md` and `summary_source_version`. The Feature DTO carries `summary_md` and `summary_source_version` (both omitted when empty). Overviews are set explicitly through that endpoint; there is no autonomous agent regeneration on canonical change.
...
The ChangeRequest DTO carries two derived fields computed on read and never persisted: `phase` and `tracker_status`. `phase` uses the richer work-item readiness model: `Intake` (no lead + no governance-chat thread), `Draft` (governance-chat thread exists but no lead artifact yet), `Review` (lead artifact exists but is not approved), `Ready` (approved lead artifact, no Context Pack), `Handoff` (Context Pack exists), and `Delivered` (latest `delivery_review` gate run for the CR has `state = pass`; overrides the pointer-derived phase). Delivered is the not-yet-archived intermediate truth: the optional `governance.auto_archive_on_delivery_pass` auto-archive keeps working unchanged, and delivered items are excluded from ready/handed-off work queues and from status attention. The latest delivery-review state is batch-loaded in one query on the list path (`DISTINCT ON (subject_id) ... ORDER BY subject_id, created_at DESC`). When related rows are unavailable, the fallback remains pointer-only (`Intake`/`Ready`/`Handoff`). `tracker_status` is the raw tracker `state.type`, empty when no tracker event. Both are populated on the single-CR read and the list path.
