...
const DefaultSkillRepoURL = "https://github.com/thanhtung2693/specgate/plugins"
...
type templateData struct {
	...
	SkillRepoURL    string // URL for npx skills add
}
...
// ServedFiles returns the sorted /plugins file allowlist.
func ServedFiles() ([]string, error) {
	pkg, err := PackageInventory()
	if err != nil {
		return nil, err
	}
	files := append([]string(nil), pkg.ServedFiles...)
	sort.Strings(files)
	return files, nil
}
...
func Render(path string, opts Options) (string, error) {
	...
	skillRepo := strings.TrimSpace(opts.SkillRepoURL)
	if skillRepo == "" {
		skillRepo = DefaultSkillRepoURL
	}
