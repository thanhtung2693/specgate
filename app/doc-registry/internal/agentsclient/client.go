// Package agentsclient is a thin HTTP client for agents-backed governance
// operations: readiness checks, quality gates, delivery review, and quick work
// creation.
//
...
// no base URL is configured, so agents-backed operations are unavailable.
...
// DefaultTimeout is generous because run-readiness may run model-judged gates.
...
// AcceptanceCriterionInput is the quick-work input shape shared with the
// agents service. It accepts string criteria as well as rich objects so
// existing callers keep working while humans author deterministic check bindings.
type AcceptanceCriterionInput struct {
	Text                string `json:"text"`
	VerificationBinding string `json:"verification_binding,omitempty"`
}

func (a AcceptanceCriterionInput) MarshalJSON() ([]byte, error) {
	text := strings.TrimSpace(a.Text)
	binding := strings.TrimSpace(a.VerificationBinding)
	if binding == "" {
...
func (a *AcceptanceCriterionInput) UnmarshalJSON(data []byte) error {
	var text string
	...
	a.Text = strings.TrimSpace(raw.Text)
	a.VerificationBinding = strings.TrimSpace(raw.VerificationBinding)
	return nil
}
...
// Client posts to agents service governance endpoints and decodes responses.
type Client struct {
	baseURL string
	http    *http.Client
}

// ResponseError carries a non-2xx agents response so callers can preserve
// validation status instead of flattening every agents failure into 500.
...
func (e *ResponseError) Error() string {
	...
	if e.Operation == "" {
		return fmt.Sprintf("agents returned %d: %s", e.StatusCode, detail)
	}
	return fmt.Sprintf("%s: agents returned %d: %s", e.Operation, e.StatusCode, detail)
}
...
func (e *ResponseError) Detail() string {
	...
	if err := json.Unmarshal([]byte(body), &payload); err == nil && payload.Detail != nil {
...
// is empty (readiness is disabled).
...
// RunLLMGates runs all model-judged quality gates for the change request's lead
// artifact and posts the verdicts to Doc Registry. Returns the raw JSON
// response from the agents service so callers can forward it unchanged.
...
// submitted by a developer in the IDE. The agents service uses supplied ACs or
// drafts them via LLM, creates the feature+CR, and returns a handoff-ready item.
func (c *Client) CreateQuickWorkItem(ctx context.Context, title, description, issueURL, issueKey, featureKey, featureName string, acceptanceCriteria []AcceptanceCriterionInput, createdBy string, workspaceID string) (map[string]any, error) {
	if c == nil {
...
func (c *Client) postJSON(ctx context.Context, url string, body []byte, errPrefix string) ([]byte, error) {
	url = withWorkspaceQuery(ctx, url)
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		...
		return nil, &ResponseError{Operation: errPrefix, StatusCode: resp.StatusCode, Body: snippet}
	}
	return respBody, nil
}

func withWorkspaceQuery(ctx context.Context, rawURL string) string {
