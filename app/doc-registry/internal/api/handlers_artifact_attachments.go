...
func (h *Handlers) CreateFeatureAttachment(
	...
	if _, err := getFeatureForWorkspace(ctx, h.WorkBoard, workspaceID, featureID); err != nil {
		return nil, mapWorkBoardError("verify attachment feature", err)
	}
	...
	if kind == artifactattachment.KindLink {
		...
		if h.GovernanceFiles != nil {
			if _, err := h.GovernanceFiles.Get(governancefiles.WithWorkspace(ctx, workspaceID), fileID); err != nil {
...
func (h *Handlers) ListFeatureAttachments(
	...
	if _, err := getFeatureForWorkspace(ctx, h.WorkBoard, workspaceID, strings.TrimSpace(in.ID)); err != nil {
		return nil, mapWorkBoardError("verify attachment feature", err)
	}
	items, err := h.ArtifactAttachments.ListByFeature(ctx, workspaceID, strings.TrimSpace(in.ID))
	if err != nil {
...
func (h *Handlers) DeleteFeatureAttachment(
	...
	if err := h.ArtifactAttachments.Delete(ctx, workspaceID, strings.TrimSpace(in.ID)); err != nil {
