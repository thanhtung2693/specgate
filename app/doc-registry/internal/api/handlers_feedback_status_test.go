...
func TestUpdateGovernanceFeedbackEventStatus(t *testing.T) {
	t.Parallel()
	ctx := integrations.WithWorkspace(context.Background(), "feedback-test")
	cases := []struct {
		name       string
		input      string // canonical wire status
		wantStored string // persisted lifecycle value
		wantWire   string // canonical status in the response
	}{
		{"resolve", "accepted", "accepted", "accepted"},
		{"dismiss", "rejected", "rejected", "rejected"},
	}
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			in := &UpdateGovernanceFeedbackEventStatusInput{ID: ev.ID, WorkspaceID: "feedback-test"}
			...
			// No get-by-id on the repo — reload via the list.
	...
	t.Run("invalid status is rejected", func(t *testing.T) {
		...
		ev, _ := intRepo.CreateGovernanceFeedbackEvent(ctx, integrations.GovernanceFeedbackEvent{
			EventType: integrations.FeedbackEventPRMerged, Status: integrations.FeedbackStatusReceived,
		})
		in := &UpdateGovernanceFeedbackEventStatusInput{ID: ev.ID, WorkspaceID: "feedback-test"}
