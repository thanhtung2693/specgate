...
const gateTaskPipelineWorkspaceID = "workspace-gate-pipeline"
...
func TestGateTaskPipeline(t *testing.T) {
	...
	req := httptest.NewRequest(http.MethodGet, "/api/v1/gate-tasks?artifact_id="+gateTaskPipelineArtifactID+"&workspace_id="+gateTaskPipelineWorkspaceID, nil)
	...
	task, err := store.CreateTask(ctx, policy.GateTaskRecord{
		WorkspaceID:    gateTaskPipelineWorkspaceID,
		...
		PolicyDigest:   "sha256:test-profile-digest",
	...
	req = httptest.NewRequest(http.MethodGet, "/api/v1/gate-tasks/"+task.ID+"?workspace_id="+gateTaskPipelineWorkspaceID, nil)
	w = httptest.NewRecorder()
	srv.ServeHTTP(w, req)
	if w.Code != http.StatusOK {
	...
	resultPayload := map[string]any{
		...
		"input_digest": "sha256:test-artifact-digest",
		"state":        "pass",
	...
	req = httptest.NewRequest(http.MethodPost, "/api/v1/gate-tasks/"+task.ID+"/result?workspace_id="+gateTaskPipelineWorkspaceID, bytes.NewReader(submitBody))
	...
	req = httptest.NewRequest(http.MethodPost, "/api/v1/gate-tasks/"+task2.ID+"/result?workspace_id="+gateTaskPipelineWorkspaceID, bytes.NewReader(staleBody))
	req.Header.Set("Content-Type", "application/json")
	w = httptest.NewRecorder()
	srv.ServeHTTP(w, req)
