...
func (u *pipelineFeatureUpserter) UpsertFeatureByKey(_ context.Context, key, name string) (*workboard.Feature, error) {
	if f, ok := u.features[key]; ok {
		return f, nil
	}
	f := &workboard.Feature{ID: "feat-" + key, Key: key, Name: name}
	u.features[key] = f
	return f, nil
}
