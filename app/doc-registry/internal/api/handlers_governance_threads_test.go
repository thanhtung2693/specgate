...
func TestGovernanceThreads_ListUpsertAndDelete(t *testing.T) {
	...
	for _, row := range []struct {
		...
		rr := putJSON(t, srv, "/governance/threads/"+row.id,
			`{"workspace_id":"ws-a","title":"`+row.title+`","preview":"preview","updated_at":"`+row.updated+`"}`)
		if rr.Code != 200 {
			t.Fatalf("upsert %s status=%d body=%s", row.id, rr.Code, rr.Body.String())
		}
	}

	list := getReq(t, srv, "/governance/threads?workspace_id=ws-a&limit=1")
	if list.Code != 200 {
	...
	del := deleteReq(t, srv, "/governance/threads/thread-2?workspace_id=ws-a")
	if del.Code != 204 {
		t.Fatalf("delete status=%d body=%s", del.Code, del.Body.String())
	}

	list = getReq(t, srv, "/governance/threads?workspace_id=ws-a&limit=10")
	body = list.Body.String()
...
func TestGovernanceThreads_CrossWorkspaceIsNotFound(t *testing.T) {
	t.Parallel()
	srv := newGovernanceThreadsTestServer(t)
	seed := putJSON(t, srv, "/governance/threads/thread-a", `{"workspace_id":"ws-a","title":"A"}`)
	if seed.Code != http.StatusOK {
		t.Fatalf("seed status=%d body=%s", seed.Code, seed.Body.String())
	}
	wrong := putJSON(t, srv, "/governance/threads/thread-a", `{"workspace_id":"ws-b","title":"B"}`)
	if wrong.Code != http.StatusNotFound {
		t.Fatalf("cross-workspace upsert status=%d body=%s", wrong.Code, wrong.Body.String())
	}
	list := getReq(t, srv, "/governance/threads?workspace_id=ws-b")
	if list.Code != http.StatusOK || !strings.Contains(list.Body.String(), `"total":0`) {
		t.Fatalf("cross-workspace list status=%d body=%s", list.Code, list.Body.String())
	}
	del := deleteReq(t, srv, "/governance/threads/thread-a?workspace_id=ws-b")
	if del.Code != http.StatusNotFound {
		t.Fatalf("cross-workspace delete status=%d body=%s", del.Code, del.Body.String())
	}
}
