...
type identityWorkspaceMembersInput struct {
	ID              string `path:"id"`
	CurrentUserID   string `query:"current_user_id"`
	CurrentUsername string `query:"current_username"`
}

type identityWorkspaceMembersOutput struct {
	Body struct {
		Workspace   identity.Workspace               `json:"workspace"`
		CurrentUser identity.User                    `json:"current_user,omitempty"`
		Members     []identity.WorkspaceMemberDetail `json:"members"`
	} `json:"body"`
}
...
func (h *Handlers) BootstrapIdentity(ctx context.Context, in *identityBootstrapInput) (*identitySelectionOutput, error) {
	...
	if h.SeedWorkspaceSkills != nil {
		if err := h.SeedWorkspaceSkills(ctx, selection.Workspace.ID); err != nil {
			return nil, huma.Error500InternalServerError("seed workspace Skills", err)
		}
	}
...
func (h *Handlers) ListWorkspaceMembers(ctx context.Context, in *identityWorkspaceMembersInput) (*identityWorkspaceMembersOutput, error) {
	if err := h.requireService(h.Identity, "identity"); err != nil {
		return nil, err
	}
	workspace, err := h.Identity.GetWorkspace(ctx, in.ID)
	...
	members, err := h.Identity.ListWorkspaceMembers(ctx, workspace.ID)
	if err != nil {
		return nil, err
	}
	currentUserID := strings.TrimSpace(in.CurrentUserID)
	currentUsername := strings.TrimSpace(in.CurrentUsername)
	for i := range members {
		if currentUserID != "" && strings.EqualFold(members[i].UserID, currentUserID) {
			members[i].Current = true
			continue
		}
		if currentUsername != "" && strings.EqualFold(members[i].Username, currentUsername) {
			members[i].Current = true
		}
	}
	out := &identityWorkspaceMembersOutput{}
	out.Body.Workspace = *workspace
	out.Body.CurrentUser = identity.User{ID: currentUserID, Username: currentUsername}
