...
import (
	...
	repository "github.com/specgate/doc-registry/internal/repository"
)
...
type GetRepoFileInput struct {
	WorkspaceID string `query:"workspace_id" required:"true" doc:"Selected workspace ID"`
...
// (integrations.ListGitLabRepoConfigs), not a separate server catalog.
func (h *Handlers) GetRepoFile(ctx context.Context, in *GetRepoFileInput) (*GetRepoFileResponse, error) {
	...
	provider := repository.NewGitLabProvider(repository.GitLabConfig{
		APIURL:     repository.NormalizeGitLabAPIURL(matched.APIURL),
		Token:      matched.Token,
