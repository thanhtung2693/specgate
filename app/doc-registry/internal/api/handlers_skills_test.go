...
func TestSkills_CRUD(t *testing.T) {
	...
	createIn.Body.WorkspaceID = "ws-a"
	...
	list, err := h.ListSkills(ctx, &ListSkillsInput{WorkspaceID: "ws-a"})
	if err != nil {
		t.Fatal(err)
	}
	...
	list2, err := h.ListSkills(ctx, &ListSkillsInput{WorkspaceID: "ws-a"})
	if err != nil {
		t.Fatal(err)
	}
	...
	del, err := h.DeleteSkill(ctx, &DeleteSkillInput{ID: id, WorkspaceID: "ws-a"})
	if err != nil {
		t.Fatal(err)
	}
	if !del.Body.OK {
	...
	list3, err := h.ListSkills(ctx, &ListSkillsInput{WorkspaceID: "ws-a"})
	if err != nil {
		t.Fatal(err)
	}
...
func TestCreateSkill_Validation(t *testing.T) {
	...
	createIn2.Body.WorkspaceID = "ws-a"
