...
func TestDerivePublicBaseURL(t *testing.T) {
	tests := []struct {
		...
		{name: "tls terminated here", host: "registry.example", tls: true, want: "https://registry.example"},
		{
			...
			host:    "127.0.0.1:8080",
			headers: map[string]string{"X-Forwarded-Proto": "https", "X-Forwarded-Host": "app.specgate.example"},
			want:    "https://app.specgate.example",
		},
	}
...
func TestOAuthCallbackBaseMiddleware_OverrideElseDerived(t *testing.T) {
	...
	if got := capture("https://app.specgate.example", "127.0.0.1:8080"); got != "https://app.specgate.example" {
