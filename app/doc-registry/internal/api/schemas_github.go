...
// gitHubWebhookInput is the GitHub peer of gitLabWebhookInput. GitHub signs the
// raw body with HMAC-SHA256, so the service must see the exact bytes — RawBody
// preserves them. Auth is the X-Hub-Signature-256 header (verified against the
// integration's recoverable secret), not a token compare.
type gitHubWebhookInput struct {
	ID               string `path:"id" doc:"integration id"`
	XGitHubEvent     string `header:"X-GitHub-Event" doc:"GitHub event name; only pull_request is processed"`
	XGitHubDelivery  string `header:"X-GitHub-Delivery" doc:"GitHub delivery UUID (replay/dedup key)"`
	XHubSignature256 string `header:"X-Hub-Signature-256" doc:"HMAC-SHA256 of the body as sha256=<hex>; verified against the per-integration GitHub webhook secret"`
	RawBody          []byte
}
