...
func TestInstanceInstallerPinsServerWithoutCredentials(t *testing.T) {
	...
	if strings.Contains(body, "API_KEY") || strings.Contains(body, "Authorization") {
		t.Fatal("CLI installer must not retrieve credentials")
	}
	if strings.Contains(body, "skipping verification") {
		t.Fatal("CLI installer must fail closed when checksum tooling is unavailable")
	}
	if !strings.Contains(body, "Cannot verify the download") {
		t.Fatal("CLI installer must explain the checksum-tool requirement")
	}
}
