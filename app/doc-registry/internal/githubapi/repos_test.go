...
func TestListRepos_RejectsOversizedResponse(t *testing.T) {
	t.Parallel()
	...
	_, err := NewClient(ClientConfig{APIURL: srv.URL, Token: "token"}).ListRepos(context.Background(), "", 50)
	if err == nil || !strings.Contains(err.Error(), "exceeds") {
		t.Fatalf("ListRepos() error = %v, want response limit", err)
	}
}
