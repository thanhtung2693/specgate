...
type Store interface {
	...
	// Delete removes an unreferenced row regardless of status. Returns the
	// deleted row's object_key so callers can issue best-effort object cleanup.
	// Returns ErrNotFound if the row does not exist or ErrReferenced when an
	// feature attachment or defensive artifact object-key reference still needs
	// the file.
