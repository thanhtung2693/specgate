...
type Explanation struct {
	...
	// PolicyLineage records the automatic policy chain that produced this snapshot.
...
func ExplainSnapshot(p ResolvedProfile) Explanation {
	return Explanation{
		...
		Summary:         approvalSummary(p.EvidencePolicy),
...
func approvalSummary(evidencePolicy string) string {
	approvalDesc := approvalDesc()
	evidenceDesc := evidenceDesc(evidencePolicy)
	return approvalDesc + "; " + evidenceDesc + "."
}

func approvalDesc() string {
...
func evidenceDesc(policy string) string {
	...
	default:
		return "evidence policy unavailable"
	}
}
...
func obligationLabels(p ResolvedProfile) []string {
	...
	switch p.EvidencePolicy {
	case "corroborated_required":
		out = append(out, "Evidence must be independently corroborated (e.g., submitted commit observed on a merged PR/MR, or deterministic check bindings for every criterion).")
	}
