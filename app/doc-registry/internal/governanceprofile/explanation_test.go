...
func TestExplainSnapshot_SummaryReflectsApprovalPolicy(t *testing.T) {
	t.Parallel()
	autoProfile := ResolvedProfile{
		GovernanceLevel: GovernanceLight,
		ApprovalPolicy:  "auto",
		EvidencePolicy:  "attested_ok",
	}
	humanProfile := ResolvedProfile{
		GovernanceLevel: GovernanceStandard,
		ApprovalPolicy:  "human_required",
		EvidencePolicy:  "attested_ok",
	}
	autoExp := ExplainSnapshot(autoProfile)
	humanExp := ExplainSnapshot(humanProfile)
	if autoExp.Summary == humanExp.Summary {
		t.Fatal("auto and human_required profiles should produce different summaries")
	}
}
