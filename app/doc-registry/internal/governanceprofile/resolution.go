...
func automaticGateSkills(enabledGates []string) map[string]string {
	...
	out := map[string]string{
		DeliveryReviewGateKey: "review-impl",
	}
	for _, gate := range enabledGates {
		if skill := wantRubric[gate]; skill != "" {
			out[gate] = skill
		}
	}
	return out
...
// KnownApprovalPolicies and KnownEvidencePolicies — enforced by
...
var builtInLevels = map[GovernanceLevel]Definition{
	GovernanceLight: {
		...
		GateSkills:       automaticGateSkills([]string{"spec_completeness", "acceptance_criteria_verifiable"}),
	},
	GovernanceStandard: {
		...
		ApprovalPolicy:   "human_required",
		EvidencePolicy:   "attested_ok",
		GateSkills:       automaticGateSkills([]string{"spec_completeness", "scope_clear", "acceptance_criteria_verifiable", "spec_repo_drift"}),
	},
	GovernanceEnhanced: {
		...
		GateSkills:       automaticGateSkills(sortedKnownGateKeys()),
	},
}
...
func criticalUnknown(d ImpactDeclaration) bool {
	return d.ProtectedDomainsStatus == "" || d.ProtectedDomainsStatus == TriUnknown
}
...
// ResolveBuiltInPolicy resolves the governance level and execution projection
// for a change using the built-in automatic policy. It does not consult the
// database.
//
...
func (Resolver) ResolveProfile(_ context.Context, in ResolveInput) (*ResolvedProfile, error) {
