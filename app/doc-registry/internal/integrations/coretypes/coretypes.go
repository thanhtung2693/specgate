...
// constants shared by the parent integrations package and provider subpackages
// without creating an import cycle. The parent re-exports the public types.
package coretypes
...
const (
	...
	// Linear issue signal. It augments the MR/PR delivery floor rather than
	// gating it.
...
const (
	TrackerLifecycleOpened  = "opened"
	TrackerLifecycleClosed  = "closed"
	TrackerLifecycleRemoved = "removed"
)
...
type Integration struct {
	...
	// token (Linear GraphQL token). Recoverable like the
...
type NormalizedDelivery struct {
	...
	Action      string
	RawState    string
	// TrackerLifecycle is provider-normalized structured state. The parent
	// service never interprets RawState or user-authored labels.
