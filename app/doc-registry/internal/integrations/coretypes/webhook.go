...
// the subset it cares about (GitLab: signing-token + legacy token; GitHub/Linear:
// Signature).
type InboundWebhook struct {
	EventHeader string // X-Gitlab-Event / X-GitHub-Event
	EventUUID   string // X-Gitlab-Event-UUID / X-GitHub-Delivery (dedup key)
	// GitLab Standard Webhooks signing-token headers (19.0+).
	WebhookID        string
	WebhookTimestamp string
	WebhookSignature string
	// Token is GitLab's legacy X-Gitlab-Token (verbatim secret-token fallback).
	Token string
...
// HashWebhookSecret returns the canonical at-rest representation of a webhook
// shared secret: lowercase SHA-256 hex of the trimmed input.
func HashWebhookSecret(plaintext string) string {
	sum := sha256.Sum256([]byte(strings.TrimSpace(plaintext)))
	return hex.EncodeToString(sum[:])
}

// VerifyWebhookSecret constant-time-compares a stored hash against the hash of an
// inbound candidate. Returns false when either is empty so a misconfigured
// integration cannot be unlocked by a blank token.
func VerifyWebhookSecret(storedHash string, candidate string) bool {
	if storedHash == "" || candidate == "" {
		return false
	}
	return subtle.ConstantTimeCompare([]byte(storedHash), []byte(HashWebhookSecret(candidate))) == 1
}
