...
func TestIsGitLabSigningToken(t *testing.T) {
	...
	if IsGitLabSigningToken("wh" + "sec_not-base64!!") {
...
func TestEncryptDecryptSecret_RoundTripsAndIsRandomized(t *testing.T) {
	...
	first, err := EncryptSecret("redacted-test-token")
	if err != nil {
		t.Fatalf("encrypt: %v", err)
	}
	second, err := EncryptSecret("redacted-test-token")
	if err != nil {
		t.Fatalf("encrypt: %v", err)
	}
	if first == second {
