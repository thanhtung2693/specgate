...
// when provided, Linear uses it to sign deliveries for this managed resource.
// The target.ExternalID must be the Linear team UUID.
...
func (webhookDriver) DeleteWebhook(ctx context.Context, hookID string, target coretypes.ProviderTarget) error {
  ...
  webhookDelete(id: $id) { success }
	...
	if err := linearGraphQLRequest(ctx, target.Token, target.Bearer, webhookDeleteMutation, map[string]any{"id": hookID}, &data); err != nil {
		if isGraphQLNotFoundError(err) {
			return nil
		}
		return err
	}
	if !data.WebhookDelete.Success {
		// Linear reports success=false when the requested hook is already absent.
		// Deletion is intentionally idempotent so local cleanup can be retried.
		return nil
	}
	return nil
}
