...
type (
	...
	TrackerIssue       = coretypes.TrackerIssue
	normalizedDelivery = coretypes.NormalizedDelivery
	trackerHandoff     = coretypes.Handoff
	trackerAdapter     = coretypes.TrackerAdapter
...
const (
	...
	WebhookEventPush         = coretypes.WebhookEventPush
	WebhookEventIssue        = coretypes.WebhookEventIssue
	WebhookEventTrackerIssue = coretypes.WebhookEventTrackerIssue
	WebhookEventWorkflowRun  = coretypes.WebhookEventWorkflowRun
	...
	DeliveryStateCIPassed = coretypes.DeliveryStateCIPassed
)
...
type WebhookEvent struct {
	...
	// convention: the `fixes SPECGATE-{key|id}` footer the author declared.
...
const (
	TrackerStateOpened  = "opened"
	TrackerStateClosed  = "closed"
	TrackerStateRemoved = "removed"
)
...
const (
	...
	// Compatibility aliases: coding-agent feedback uses the newer lifecycle
	// names in contracts/specs, while the current DB enum still persists the
	// legacy values. Keep both until the lifecycle migration lands.
	FeedbackStatusReceived        = FeedbackStatusPending
	FeedbackStatusAccepted        = FeedbackStatusProcessed
	FeedbackStatusRejected        = FeedbackStatusIgnored
	FeedbackStatusTriaged         = "triaged"
	FeedbackStatusProposalDrafted = "proposal_drafted"
)
...
const (
	...
	FeedbackEventContextPackStale            = "planning.context_pack_stale"
	...
	// signal augments — never gates — the git/MCP delivery floor; its payload
	...
	// FeedbackEventCIPassed is emitted when a GitHub Actions workflow_run (or
	// equivalent CI pipeline) completes successfully on a branch linked to a
	// work item. Its presence upgrades the evidence trust tier to ci_verified
	// in the delivery review. per spec §11.
	FeedbackEventCIPassed = "delivery.ci_passed"
)
...
// TrackerLink is the persisted handoff link between a tracker issue (Linear /
// GitLab / GitHub) and its work item. Keyed by (integration_id, external_key):
// external_key (the human identifier, e.g. ENG-123) is always present and stays
// lane-distinct; external_id (the provider's immutable id) is populated only
// where the create response carries it (Linear) and is a secondary correlation
// column. Distinct from the MR-shaped DeliveryLink — no resource, has a lane.
type TrackerLink struct {
	ID              string `json:"id" gorm:"column:id;primaryKey"`
	IntegrationID   string `json:"integration_id" gorm:"column:integration_id"`
	FeatureID       string `json:"feature_id" gorm:"column:feature_id"`
	ChangeRequestID string `json:"change_request_id" gorm:"column:change_request_id"`
	Lane            string `json:"lane" gorm:"column:lane"`
	ExternalID      string `json:"external_id" gorm:"column:external_id"`
	ExternalKey     string `json:"external_key" gorm:"column:external_key"`
	URL             string `json:"url" gorm:"column:url"`
	Title           string `json:"title" gorm:"column:title"`
	...
	// e.g. "In Review", "Done"); for GitLab it is the derived type-like state
	// ("started", "completed"). The value is used for dedup (emit
...
type GitLabWebhookInput struct {
	EventHeader string
	EventUUID   string
	// Standard Webhooks signing-token headers (GitLab 19.0+). WebhookSignature is
	// a space-separated list of v1,<base64> candidates.
	WebhookID        string
	WebhookTimestamp string
	WebhookSignature string
	// Token is the legacy X-Gitlab-Token secret (verbatim), used when the
	// integration/resource stored a secret token instead of a signing token.
	Token       string
	PayloadJSON string
}

type GitHubWebhookInput struct {
	EventHeader string // X-GitHub-Event (must be "pull_request")
	DeliveryID  string // X-GitHub-Delivery (replay/dedup key)
	Signature   string // X-Hub-Signature-256: sha256=<hmac>
	PayloadJSON string
}
