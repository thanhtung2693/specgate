...
func (s *Service) CreateResourceAndProvisionWebhook(ctx context.Context, integrationID string, in Resource, registryBaseURL string) (*Resource, error) {
	...
	if !webhookableResourceType(integration.Provider, created.ResourceType) {
		return created, nil
	}
...
func (s *Service) ReprovisionResourceWebhook(ctx context.Context, integrationID, resourceID, registryBaseURL string) (*Resource, error) {
	...
	if !webhookableResourceType(integration.Provider, resource.ResourceType) {
		return nil, fmt.Errorf("%w: only project/repo/team resources have webhooks", ErrValidation)
	}
	if !integration.HasAPIToken && !integration.HasOAuthToken {
...
// webhookableResourceType reports whether a resource of the given type may carry
// a managed webhook for the given provider. GitLab/GitHub use project/repo; Linear
// uses team (each webhook is team-scoped in the Linear data model). per spec §14.
func webhookableResourceType(provider, resourceType string) bool {
	switch provider {
	case ProviderLinear:
		return resourceType == ResourceTypeTeam
	default:
		return resourceType == ResourceTypeProject || resourceType == ResourceTypeRepo
	}
}
