...
func SeedDemo(ctx context.Context, deps DemoDeps) (DemoResult, error) {
	...
	if strings.TrimSpace(deps.WorkspaceID) == "" {
		return DemoResult{}, fmt.Errorf("seeding: workspace_id is required")
	}
	logger := deps.Logger
	if logger == nil {
		l := zerolog.Nop()
	...
	ctx = knowledge.WithWorkspace(ctx, workspaceID)
...
func ensureDemoIntegrations(ctx context.Context, repo *storagedb.IntegrationRepository) error {
	...
	linearID := demoScopedID(workspaceID, "integration:linear")
	linearTeamID := demoScopedID(workspaceID, "resource:linear-team")
	gitLabID := demoScopedID(workspaceID, "integration:gitlab")
	projectID := demoScopedID(workspaceID, "resource:gitlab-project")
	if _, err := repo.GetIntegration(ctx, linearID); err != nil {
	...
	if _, err := repo.GetIntegration(ctx, gitLabID); err != nil {
		if !errors.Is(err, integrations.ErrNotFound) {
	...
	if _, err := repo.GetResource(ctx, linearID, linearTeamID); err != nil {
		if !errors.Is(err, integrations.ErrNotFound) {
			return fmt.Errorf("seeding demo: get linear team resource: %w", err)
		}
		if _, err := repo.CreateResource(ctx, integrations.Resource{
			ID: linearTeamID, IntegrationID: linearID, ResourceType: integrations.ResourceTypeTeam,
			ExternalID: "demo-linear-team", ExternalKey: "SPECGATE", DisplayName: "Demo Linear Team", ConfigJSON: `{"demo":true}`,
		}); err != nil && !errors.Is(err, integrations.ErrConflict) {
			return fmt.Errorf("seeding demo: create linear team resource: %w", err)
		}
	}
	if _, err := repo.GetResource(ctx, gitLabID, projectID); err != nil {
		if !errors.Is(err, integrations.ErrNotFound) {
...
func seedDemoFeature(ctx context.Context, deps DemoDeps, seed demoFeature, result *DemoResult) error {
	...
	canonical, err := publishDemoArtifact(ctx, deps.Artifacts, workspaceID, feature.ID, "v1.0", artifact.StatusApproved, artifact.ArtifactPhasePhase1, seed.Name, seed.ServiceName, createdBy)
	if err != nil {
	...
	if seed.Status == workboard.FeatureStatusCandidate {
		if _, err := publishDemoArtifact(ctx, deps.Artifacts, workspaceID, feature.ID, "draft-review", artifact.StatusDraft, artifact.ArtifactPhasePhase1, seed.Name+" Draft", seed.ServiceName, createdBy); err != nil {
			return fmt.Errorf("seeding demo: publish draft artifact for %s: %w", seed.Key, err)
		}
		result.ArtifactsPublished++
	}
	if seed.Knowledge != nil {
		if err := createDemoKnowledge(ctx, deps.Knowledge, workspaceID, feature.ID, *seed.Knowledge, createdBy); err != nil {
	...
	for _, crSeed := range seed.CRs {
		...
		if crSeed.WithLead {
			if crSeed.LeadVersion != "" || crSeed.LeadStatus != "" {
				...
				lead, err := publishDemoArtifact(ctx, deps.Artifacts, workspaceID, feature.ID, leadVersion, leadStatus, artifact.ArtifactPhasePhase1, crSeed.Title+" Working Spec", seed.ServiceName, createdBy)
				if err != nil {
...
func demoFeaturesForWorkspace(workspaceID string) []demoFeature {
	features := demoFeatures()
	for i := range features {
		features[i].ID = demoScopedID(workspaceID, "feature:"+features[i].ID)
		if features[i].Knowledge != nil {
			features[i].Knowledge.DocumentID = demoScopedID(workspaceID, "knowledge:"+features[i].Knowledge.DocumentID)
		}
		for j := range features[i].CRs {
			features[i].CRs[j].ID = demoScopedID(workspaceID, "change-request:"+features[i].CRs[j].ID)
			for k := range features[i].CRs[j].AC {
				if id, ok := features[i].CRs[j].AC[k]["id"].(string); ok && id != "" {
...
func demoScopedID(workspaceID, name string) string {
	return uuid.NewSHA1(uuid.NameSpaceOID, []byte("specgate-demo/"+workspaceID+"/"+name)).String()
}
...
func publishDemoArtifact(
	...
	confidence := 0.86
	ambiguity := 0.18
	resolved, err := governanceprofile.ResolveBuiltInPolicy(governanceprofile.ResolveInput{
		RequestType: string(artifact.RequestTypeChangeRequest),
		ImpactLevel: string(artifact.ImpactLevelMedium),
		ImpactDeclaration: governanceprofile.ImpactDeclaration{
			ProtectedDomainsStatus: governanceprofile.TriNo,
		},
	})
	if err != nil {
		return nil, fmt.Errorf("resolve demo governance policy: %w", err)
	}
	resolved.SnapshotSchemaVersion = governanceprofile.SnapshotSchemaPolicyV1
	policySnapshot, err := resolved.SnapshotJSON()
	if err != nil {
		return nil, fmt.Errorf("serialize demo governance policy: %w", err)
	}
	return svc.Publish(ctx, artifact.PublishInput{
		...
		Documents: []artifact.DocumentInput{
			{Path: "product-intent.md", Role: string(artifact.RoleSpec), Content: []byte("# " + title + " Intent\n\nDemo user intent, goals, non-goals, and review notes.\n")},
			{Path: "contract.md", Role: string(artifact.RoleSpec), Content: []byte("# " + title + " Contract\n\nDemo contract, lifecycle notes, API expectations, and edge cases.\n")},
			{Path: "implementation.md", Role: string(artifact.RolePlan), Content: []byte("# Implementation\n\n- Preserve registry contract\n- Review UI states\n- Add focused coverage\n")},
			{Path: "verification.md", Role: string(artifact.RoleVerification), Content: []byte("# Verification\n\n- Verify happy path\n- Verify stale and delivery states\n")},
			{Path: "rollout.md", Role: string(artifact.RoleReference), Content: []byte("# Rollout\n\nShip behind normal review flow.\n")},
			{Path: "risks.md", Role: string(artifact.RoleReference), Content: []byte("# Risks\n\nDemo data should stay local-only.\n")},
			{Path: "manifest.json", Role: string(artifact.RoleReference), Content: []byte(`{"source":"seed-demo","version":"v1"}`)},
		},
	})
}

func createDemoKnowledge(ctx context.Context, repo *storagedb.KnowledgeRepository, workspaceID, featureID string, seed demoKnowledge, uploadedBy string) error {
