...
func testWorkspaceContext() context.Context {
	return skills.WithWorkspace(context.Background(), "ws-a")
}
...
func TestSeedSkills_FreshDB(t *testing.T) {
	...
	forEachDriver(t, func(t *testing.T, name string, skillsSvc *skills.Service, settingsSvc *settings.Service) {
		...
		result, err := seeding.SeedSkills(testWorkspaceContext(), skillsSvc, settingsSvc, discardLogger())
		if err != nil {
		...
		all, err := skillsSvc.List(testWorkspaceContext())
		if err != nil {
...
func TestSeedSkills_Idempotent(t *testing.T) {
	...
	forEachDriver(t, func(t *testing.T, name string, skillsSvc *skills.Service, settingsSvc *settings.Service) {
		...
		// First run.
		if _, err := seeding.SeedSkills(testWorkspaceContext(), skillsSvc, settingsSvc, discardLogger()); err != nil {
...
func TestSeedSkills_OverwriteExistingUpdatesStarterRubrics(t *testing.T) {
	...
	forEachDriver(t, func(t *testing.T, name string, skillsSvc *skills.Service, settingsSvc *settings.Service) {
		...
		result, err := seeding.SeedSkillsWithOptions(testWorkspaceContext(), skillsSvc, settingsSvc, discardLogger(), seeding.SkillSeedOptions{
		...
		updated, err := skillsSvc.Get(testWorkspaceContext(), existing.ID)
		if err != nil {
...
func TestSeedSkills_StarterRubricsStayGateBounded(t *testing.T) {
	...
	forEachDriver(t, func(t *testing.T, name string, skillsSvc *skills.Service, settingsSvc *settings.Service) {
		...
		if !strings.Contains(strings.ToLower(byName["review-impl"].Prompt), "delivery evidence") {
...
func TestSeedSkills_StarterRubricsUseCurrentGateVerdicts(t *testing.T) {
	t.Parallel()
	forEachDriver(t, func(t *testing.T, name string, skillsSvc *skills.Service, settingsSvc *settings.Service) {
		if _, err := seeding.SeedSkills(testWorkspaceContext(), skillsSvc, settingsSvc, discardLogger()); err != nil {
			t.Fatalf("[%s] SeedSkills: %v", name, err)
		...
		for _, skill := range all {
			if strings.Contains(skill.Prompt, "needs_changes") {
				t.Errorf("[%s] %s uses retired needs_changes verdict", name, skill.Name)
			}
			if skill.Name != "review-impl" && skill.Name != "spec-review" && !strings.Contains(skill.Prompt, "needs_human_review") {
				t.Errorf("[%s] %s must route ambiguous evidence to needs_human_review", name, skill.Name)
			}
		}

		delivery, err := findSkillByName(t, skillsSvc, "review-impl")
		if err != nil {
			t.Fatalf("[%s] find review-impl: %v", name, err)
		}
		if !strings.Contains(delivery.Prompt, "needs_human_review") {
			t.Errorf("[%s] review-impl must preserve needs_human_review for unclear evidence", name)
		}
	})
}

func TestSeedSkills_RubricsMatchTheirBoundJudgmentSchemas(t *testing.T) {
	t.Parallel()
	forEachDriver(t, func(t *testing.T, name string, skillsSvc *skills.Service, settingsSvc *settings.Service) {
		if _, err := seeding.SeedSkills(testWorkspaceContext(), skillsSvc, settingsSvc, discardLogger()); err != nil {
			t.Fatalf("[%s] SeedSkills: %v", name, err)
		...
		for _, skillName := range []string{"rollout-risk", "acceptance-criteria", "task-breakdown", "prd-review"} {
			skill, err := findSkillByName(t, skillsSvc, skillName)
			if err != nil {
				t.Fatalf("[%s] find %s: %v", name, skillName, err)
			}
			if !strings.Contains(skill.Prompt, "change type") {
				t.Errorf("[%s] %s must tie not_applicable to the change type", name, skillName)
			}
			if strings.Contains(skill.Prompt, "selected profile explicitly") {
				t.Errorf("[%s] %s must not use profile exclusion as not_applicable evidence", name, skillName)
			}
		}

		spec, err := findSkillByName(t, skillsSvc, "spec-review")
		if err != nil {
			t.Fatalf("[%s] find spec-review: %v", name, err)
		}
		for _, status := range []string{"`covered`", "`partial`", "`missing`", "`not_applicable`"} {
			if !strings.Contains(spec.Prompt, status) {
				t.Errorf("[%s] spec-review must use completeness topic status %s", name, status)
			}
		}
	})
}
...
func findSkillByName(t *testing.T, svc *skills.Service, name string) (*skills.Skill, error) {
	t.Helper()
	all, err := svc.List(testWorkspaceContext())
	if err != nil {
		return nil, err
	}
	for i := range all {
