...
func TestArtifactAttachmentRepository_CreateListDelete(t *testing.T) {
	...
	forEachDriver(t, func(t *testing.T, name string, gdb *gorm.DB) {
		...
		items, err := repo.ListByFeature(ctx, "ws-test", "feature-1")
		if err != nil {
		...
		if err := repo.Delete(ctx, "ws-test", "a1"); err != nil {
			t.Fatalf("Delete a1: %v", err)
		}
		if err := repo.Delete(ctx, "ws-test", "a1"); !errors.Is(err, artifactattachment.ErrNotFound) {
			t.Fatalf("re-delete err = %v, want ErrNotFound", err)
		}
		left, _ := repo.ListByFeature(ctx, "ws-test", "feature-1")
...
// purger — otherwise an uploaded screenshot's object is deleted out from
...
func TestArtifactAttachmentRepository_PinsFileFromTTLPurge(t *testing.T) {
	...
	forEachDriver(t, func(t *testing.T, name string, gdb *gorm.DB) {
		...
		ctx := governancefiles.WithWorkspace(context.Background(), "ws-test")
...
func TestArtifactAttachmentRepository_GetNotFound(t *testing.T) {
	...
	forEachDriver(t, func(t *testing.T, name string, gdb *gorm.DB) {
		repo := NewArtifactAttachmentRepository(gdb)
		if _, err := repo.Get(context.Background(), "ws-test", "missing"); !errors.Is(err, artifactattachment.ErrNotFound) {
