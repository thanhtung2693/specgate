...
// document bytes (so PutObject/PresignGet are never exercised).
type noopObjectStore struct{}

func (noopObjectStore) PutObject(context.Context, string, []byte) error { return nil }
func (noopObjectStore) GetObject(context.Context, string) ([]byte, error) {
	return nil, errors.New("noop")
}
func (noopObjectStore) PresignGet(context.Context, string) (string, error) { return "", nil }
...
func newPublishService(gdb *gorm.DB) *artifact.RegistryService {
	...
	return artifact.NewService(repo, noopObjectStore{}, objectKey, time.Minute)
}
...
// stale base at the driver level before the row is written, across both drivers.
