...
// history shows every executor with its origin, while the approval aggregate
// (a separate query) stays platform-only.
func TestListReadinessRunsIncludesAgentAttestedRuns(t *testing.T) {
	forEachDriver(t, func(t *testing.T, _ string, gdb *gorm.DB) {
		...
		runs := []workboard.GateRun{
			{ID: "run-platform", SubjectKind: workboard.GateRunSubjectArtifact, SubjectID: "art-x", Gate: "scope_clear", State: workboard.NextActionStatePass, Hint: "bounded", Executor: workboard.GateRunExecutorPlatform, CreatedAt: now.Add(-2 * time.Hour)},
			{ID: "run-agent", SubjectKind: workboard.GateRunSubjectArtifact, SubjectID: "art-x", Gate: "spec_completeness", State: workboard.NextActionStateWarn, Hint: "thin risks", Executor: workboard.GateRunExecutorIDEAgent, CreatedAt: now.Add(-1 * time.Hour)},
		}
		for i := range runs {
