...
func TestSkillRepository_CreateListGetUpdateDelete(t *testing.T) {
	forEachDriver(t, func(t *testing.T, name string, gdb *gorm.DB) {
		...
		after, err := repo.Get(ctx, "skill-1")
		if err != nil {
			t.Fatal(err)
		}
		...
		if err := repo.Delete(ctx, "skill-1"); err != nil {
			t.Fatalf("delete: %v", err)
		}
		if _, err := repo.Get(ctx, "skill-1"); err != gorm.ErrRecordNotFound {
...
func TestSkillRepository_Delete_NotFound(t *testing.T) {
	forEachDriver(t, func(t *testing.T, name string, gdb *gorm.DB) {
		repo := NewSkillRepository(gdb)
		if err := repo.Delete(context.Background(), "nonexistent"); err != gorm.ErrRecordNotFound {
