...
// Governance stats projection queries. Both
...
// human-approved delivery can auto-archive its CR, and those items still count
// as reviewed work. Gate runs come from the unified gate_runs table,
...
func (r *WorkBoardRepository) ListGateRunsForStats(
	...
	q := r.db.WithContext(ctx).
		...
		// Join workspace ownership too: gate_runs has no FK to change_requests,
		// so a malformed row must not appear in another workspace's projection.
		Joins("JOIN change_requests cr ON cr.id = gr.subject_id AND cr.workspace_id = gr.workspace_id").
...
func (r *WorkBoardRepository) ListAmbiguityFeedbackForStats(
	...
	q := r.db.WithContext(ctx).
		...
		// Feedback has the same denormalized CR link as gate runs; require both
		// records to agree on workspace before projecting it into stats.
		Joins("JOIN change_requests cr ON cr.id = fe.change_request_id AND cr.workspace_id = fe.workspace_id").
