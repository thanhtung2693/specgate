...
func seedStatsFixtures(t *testing.T, gdb *gorm.DB, now time.Time) {
	...
	runs := []workboard.GateRun{
		{ID: "run-recent-ws1", SubjectKind: workboard.GateRunSubjectChangeRequest, SubjectID: "cr-ws1", Gate: "delivery_review", State: workboard.NextActionStatePass, Hint: "clean", Executor: workboard.GateRunExecutorPlatform, CreatedAt: now.Add(-2 * time.Hour)},
		{ID: "run-recent-ws2", SubjectKind: workboard.GateRunSubjectChangeRequest, SubjectID: "cr-ws2", Gate: "scope_clear", State: workboard.NextActionStateFail, Hint: "unclear", Executor: workboard.GateRunExecutorPlatform, CreatedAt: now.Add(-3 * time.Hour)},
		{ID: "run-old-ws1", SubjectKind: workboard.GateRunSubjectChangeRequest, SubjectID: "cr-ws1", Gate: "delivery_review", State: workboard.NextActionStateFail, Hint: "stale", Executor: workboard.GateRunExecutorPlatform, CreatedAt: now.Add(-40 * 24 * time.Hour)},
		...
		// (subject_kind filter).
		{ID: "run-artifact-noise", SubjectKind: workboard.GateRunSubjectArtifact, SubjectID: "cr-ws1", Gate: "spec_completeness", State: workboard.NextActionStatePass, Hint: "readiness", Executor: workboard.GateRunExecutorPlatform, CreatedAt: now.Add(-time.Hour)},
	}
	for i := range runs {
	...
	events := []integrations.GovernanceFeedbackEvent{
		{ID: "evt-ambiguity-ws1", ChangeRequestID: "cr-ws1", EventType: integrations.FeedbackEventCodingAgentBlockedAmbiguity, Reason: "which flag?", PayloadJSON: "{}", Status: integrations.FeedbackStatusReceived, CreatedAt: now.Add(-4 * time.Hour), UpdatedAt: now},
		{ID: "evt-ambiguity-ws2", ChangeRequestID: "cr-ws2", EventType: integrations.FeedbackEventCodingAgentBlockedAmbiguity, Reason: "which env?", PayloadJSON: "{}", Status: integrations.FeedbackStatusReceived, CreatedAt: now.Add(-5 * time.Hour), UpdatedAt: now},
		{ID: "evt-other-type", ChangeRequestID: "cr-ws1", EventType: integrations.FeedbackEventCodingAgentCompleted, Reason: "done", PayloadJSON: "{}", Status: integrations.FeedbackStatusReceived, CreatedAt: now.Add(-4 * time.Hour), UpdatedAt: now},
		{ID: "evt-old", ChangeRequestID: "cr-ws1", EventType: integrations.FeedbackEventCodingAgentBlockedAmbiguity, Reason: "old", PayloadJSON: "{}", Status: integrations.FeedbackStatusReceived, CreatedAt: now.Add(-40 * 24 * time.Hour), UpdatedAt: now},
	}
	for i := range events {
