...
func (s *Store) Upsert(ctx context.Context, points []knowledge.VectorPoint) error {
	...
	for _, p := range points {
		workspaceID, _ := p.Payload["workspace_id"].(string)
		if strings.TrimSpace(workspaceID) == "" {
			return fmt.Errorf("pgvector: workspace_id is required")
		}
...
func (s *Store) Search(ctx context.Context, in knowledge.VectorSearch) ([]knowledge.VectorResult, error) {
	if strings.TrimSpace(in.WorkspaceID) == "" {
		return nil, fmt.Errorf("pgvector: workspace_id is required")
	}
	limit := in.Limit
	if limit <= 0 {
		limit = 8
	...
	if in.WorkspaceID != "" {
		where = append(where, fmt.Sprintf("payload->>'workspace_id' = $%d", idx))
