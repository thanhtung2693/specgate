...
export function reviewTone(item: WorkItem): Tone {
  if (item.deliveryVerdict && item.deliveryVerdict !== "pass") return "warning"
  if (item.delivery === "needs_changes" || item.gate === "fail") return "warning"
  if (item.delivery === "ready" || item.delivery === "passed" || item.delivery === "accepted") return "success"
  return "neutral"
}
...
// Server-derived board phase for change requests whose authoritative delivery review
// passed. The phase arrives through `lifecycle`; servers that omit it keep the
// previous board grouping.
