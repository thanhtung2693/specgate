...
describe("governance stats data adapter", () => {
  ...
  it("does not issue an unscoped stats request", async () => {
    const fetchMock = vi.fn()
    vi.stubGlobal("fetch", fetchMock)

    await expect(fetchGovernanceStats("http://registry.test")).rejects.toThrow("workspaceId is required")
  ...
  it("throws on non-ok stats responses", async () => {
    ...
    await expect(fetchGovernanceStats("http://registry.test", "ws-1")).rejects.toThrow("stats request failed: 503")
  })
})
