...
A Feature Summary is a generated, reviewable product-memory rollup for one Feature across many artifacts and work items. It is not the implementation contract. It summarizes current behavior, why the feature exists, canonical artifact, linked work items, known gaps, delivery evidence, and related knowledge.

The canonical file key is `feature-summary.md` when persisted inside an artifact-like bundle. Feature summary updates are generated by the governance-ops, reviewed by a human, and only then become the latest Feature summary. Generated summaries are drafts, not canonical memory.

Lifecycle:

- `draft` — generated summary awaiting review.
- `reviewed` — human-approved summary.
- `current` — reviewed summary that matches latest linked artifacts and delivery feedback.
- `stale` — reviewed summary that may be outdated because linked artifact, implementation feedback, or related knowledge changed.
...
All feedback events are append-only `governance_feedback_events` rows. Feedback never mutates approved artifacts directly. Actionable feedback moves through `received -> triaged -> proposal_drafted -> accepted/rejected/ignored` and creates an artifact-edit proposal tagged with `source_kind=feedback_event` and `source_id=<governance_feedback_events.id>`.
...
Closes the spine's right half: a signal (a delivery/feedback event, a failing gate) becomes a **reviewed** artifact-update proposal that a human approves or rejects — feedback never rewrites an approved artifact directly.

- A **proposal is an artifact edit session** tagged with its origin: `source_kind` (`feedback_event`, `gate`, …) + `source_id`. Ordinary edit sessions have empty source.
- The proposal's **content (the proposed hunks) is drafted upstream** by the governance-ops, which creates the edit session and fills the working files. Doc Registry owns the **governance**, not the drafting.
...
- Deferred: reconciling the originating signal's status on verdict (cross-package), and the agents-side drafting.
